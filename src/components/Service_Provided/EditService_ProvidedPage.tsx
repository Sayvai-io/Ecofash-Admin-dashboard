...
const EditService_ProvidedPage = ({ 
  ...
  useEffect(() => {
    const fetchServiceData = async () => {
      ...
      setEditService(data);
      setImagePreview(data.bg_image); // Updated field name
    };
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      setIsDirty(true);
      setImageUploadActive({ ...imageUploadActive, bg_image: true }); // Activate upload button
    }
  };
  ...
  const handleRemoveImage = () => {
    ...
    setIsDirty(true);
    setImageUploadActive({ ...imageUploadActive, bg_image: false }); // Deactivate upload button
  };
  ...
  const handleUpdate = async () => {
    ...
    let imageUrl = editService.bg_image; // Default to existing image
    ...
    } else if (editService.bg_image === null) {
      // If the image was removed, set imageUrl to null
      imageUrl = null;
    }

    updatedService.bg_image = imageUrl; // Update the bg_image in the updatedService object
