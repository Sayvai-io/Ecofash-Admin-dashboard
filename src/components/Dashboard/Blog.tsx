...
//       // Add blog post
//       const result = await addBlogPost(title, content, imageUrl, tags);
...
const Blog: React.FC = () => {
  ...
  // Insert blog post into Supabase database
  const addBlogPost = async (title: string, content: string, tags: string[]) => {
    console.log('Adding blog post...', { title, content, tags });
    try {
      const { data, error } = await supabase
        .from('blogs')
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      // Add blog post
      const result = await addBlogPost(title, content, tags);
      console.log('Blog post added, result:', result);
