...
const Contact_Address_PagePreview = ({  
  ...
  useEffect(() => {
    const fetchCountriesAndAddresses = async () => {
      const { data: countriesData, error: countriesError } = await supabase
        .from("country")
        .select(`
          id,
          country_name,
          addresses:address (id, full_address, email, contact_no) 
        `);
  ...
  return (
    <div className="max-w-5xl mx-auto pb-6 mt-2 bg-white border rounded-lg shadow-lg p-10">
      <div className="flex border-b mb-8 mt-4 justify-between items-center">
        <h1 className="text-2xl text-gray-700 font-bold mb-4">Addresses Preview</h1>
        <button 
        ...
        {countries.map((country, countryIndex) => (
          <div key={countryIndex} className="border rounded-md bg-gray-100 p-4 mb-4">
            ...
            {country.addresses.length > 0 ? (
              country.addresses.map((address: Address, addressIndex: number) => (
...
export default Contact_Address_PagePreview;